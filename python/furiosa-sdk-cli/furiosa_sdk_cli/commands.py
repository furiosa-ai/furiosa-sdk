...
class Version(Command):
    ...
    def run(self) -> int:
        ...
        r = requests.get(request_url, headers=http.DEFAULT_HEADERS, auth=ApiKeyAuth(self.session))
        ...
        if r.status_code == 200:
            ...
            print(
                "Server version: {} (rev: {} built_at: {})".format(
                    server_version, server_revision, server_build_time
                )
            )
...
class Compile(Command):
    ...
    def run(self) -> int:
        ...
        with open(source_path, "rb") as file:
            task = client.submit_compile(
                source=file, compiler_config=compiler_config, target_npu_spec=target_npu_spec
            )
        ...
        if task.is_succeeded():
            ...
            with open(output_path, "wb") as output_file:
                ...
                ms_elapsed = 100
                self.print_message(
                    "{} has been generated (elapsed: {} ms)".format(output_path, ms_elapsed)
                )
            ...
            if self.args.compiler_report is not None:
                ...
                with open(compiler_report_path, "w") as compiler_report_file:
                    ...
                    self.print_message(
                        "the compiler report has been written to {}".format(compiler_report_path)
                    )
            ...
            if self.args.mem_alloc_report is not None:
                ...
                with open(self.args.mem_alloc_report, "w") as mem_alloc_report_file:
                    ...
                    self.print_message(
                        "the memory allocation report has been written to {}".format(
                            mem_alloc_report_path
                        )
                    )
        else:
...
class Perf(Command):
    ...
    def run(self) -> int:
        ...
        r = requests.post(
            request_url, data=multi_parts, headers=headers, auth=ApiKeyAuth(self.session)
        )
...
class Optimize(Command):
    ...
    def optimize(session, model: bytes, model_path: str = None) -> bytes:
        ...
        multi_parts = MultipartEncoder(
            fields={"source": (model_path, model, "application/octet-stream")}
        )
        ...
        r = requests.post(request_url, data=multi_parts, headers=headers, auth=ApiKeyAuth(session))
    ...
    def run(self) -> int:
        ...
        with open(source_path, "rb") as model, open(output_path, "wb") as output_file:
...
class BuildCalibrationModel(Command):
    ...
    @staticmethod
    def build_calibration_model(session, model: bytes, model_path: str = None) -> bytes:
        model_path = model_path or "model.onnx"
        multi_parts = MultipartEncoder(
            fields={"source": (model_path, model, "application/octet-stream")}
        )
    ...
    def run(self) -> int:
        ...
        with open(source_path, "rb") as model, open(output_path, "wb") as output_file:
            model = BuildCalibrationModel.build_calibration_model(
                self.session, model.read(), model_path=source_path
            )
            output_file.write(model)
...
class Quantize(Command):
    ...
    def run(self) -> int:
        ...
        with open(source_path, "rb") as model, open(dynamic_ranges, "r") as dynamic_ranges, open(
            output_path, "wb"
        ) as output_file:
            dynamic_ranges = json.load(dynamic_ranges)
            model = Quantize.quantize(
                self.session, model.read(), dynamic_ranges, model_path=source_path
            )
            output_file.write(model)
...
class ValidateModel(Command):
    ...
    def run(self) -> int:
        ...
        result = subprocess.run(
            ["furiosa-validate", self.args.model_path],
            env=this_envs,
            capture_output=True,

...
class CommandTests(unittest.TestCase):
    ...
    def test_compile_with_compiler_config(self):
        result = subprocess.run(
            ["furiosa", "-d", "-v", "compile", self.mnist_model, "--config", self.compiler_config],
            capture_output=True,
        )
        self.assertEqual(0, result.returncode, result.stderr)
        self.assertIn("output.enf has been generated", str(result.stdout))
    ...
    def test_toolchain_list(self):
        result = subprocess.run(["furiosa", "-v", "toolchain", "list"], capture_output=True)

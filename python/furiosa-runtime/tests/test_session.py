...
class TestSession(unittest.TestCase):
    ...
    def test_run_invalid_input(self):
        ...
        self.assertRaises(
            ...
            # errors.InvalidInput,
            (errors.NativeException, TypeError),
            lambda: sess.run(np.zeros((1, 28, 28, 1), dtype=np.float16)),
        )

        self.assertRaises(
            # FIXME: Add specific error variants to furiosa-native-runtime
            # errors.InvalidInput,
            (errors.NativeException, ValueError),
...
class TestSessionClosed(unittest.TestCase):
    def test_session_closed(self):
        ...
        self.assertRaises(errors.NativeException, lambda: sess.inputs())
        self.assertRaises(errors.NativeException, lambda: sess.input_num())
        self.assertRaises(errors.NativeException, lambda: sess.outputs())
        self.assertRaises(errors.NativeException, lambda: sess.output_num())
        self.assertRaises(errors.NativeException, lambda: sess.summary())
        self.assertRaises(errors.NativeException, lambda: sess.run(None))
    ...
    def test_async_session_closed(self):
        ...
        self.assertRaises(errors.NativeException, lambda: submitter.inputs())
        self.assertRaises(errors.NativeException, lambda: submitter.input_num())
        self.assertRaises(errors.NativeException, lambda: submitter.outputs())
        self.assertRaises(errors.NativeException, lambda: submitter.output_num())
        self.assertRaises(errors.NativeException, lambda: submitter.summary())
        queue.close()
    ...
    def test_queue_when_session_closed(self):
        ...
        self.assertRaises(errors.NativeException, lambda: queue.recv())
        submitter.close()

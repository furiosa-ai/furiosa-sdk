...
def test_run(mnist_onnx, mnist_images):
    runner = session.create(model=mnist_onnx)
...
def test_create(mnist_onnx):
    assert session.create(mnist_onnx, worker_num=1, compiler_config={"allow_precision_error": True})
...
def test_batch(mnist_onnx):
    ...
    with session.create(mnist_onnx, batch_size=4) as runner:
        runner.input(0).shape == (4, 1, 28, 28)

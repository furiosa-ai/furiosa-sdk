...
class Pattern_2(Pattern_1, abc.ABC):
    ...
    def make_new_node(self, matched_nodes):
        ...
        return [
            self.make_node(
                "Mul",
                [node.input[0], node.input[0] + "_bn_multiplier"],
    ...
    def make_new_init(self, matched_nodes):
        ...
        return [
            self.make_initializer_from_array(
                multiplier.reshape(num_features, -1, 1, 1), name=node.input[0] + "_bn_multiplier"
            ),
            self.make_initializer_from_array(
                shifter.reshape(num_features, -1, 1, 1), name=node.input[0] + "_bn_shifter"
            ),
        ]

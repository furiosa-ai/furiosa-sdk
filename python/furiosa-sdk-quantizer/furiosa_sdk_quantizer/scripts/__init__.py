...
def main():
    ...
    if args.command == "export_spec":
        ...
        post_training_quantization_with_random_calibration(
            args_map["i"], args_map["o"], args_map["n"]
        )
...
def parse_args():
    ...
    export_spec_cmd.add_argument(
        "-i", type=str, help="Path to Model file (tflite, onnx are supported)"
    )
    export_spec_cmd.add_argument("-o", type=str, help="Path to Output file")

    build_calibration_model_cmd = subparsers.add_parser(
        "build_calibration_model", help="build calibrate model help"
    )
    ...
    post_training_quantization_with_random_calibration.add_argument(
        "-o", type=str, help="Path to Output file"
    )
    post_training_quantization_with_random_calibration.add_argument(
        "-n", type=int, help="The number of random data"
    )

    calibrate_with_random_cmd = subparsers.add_parser(
        "calibrate_with_random", help="Output: dynamic ranges"
    )

...
위 요구 사항이 준비된 경우 Python 환경이 준비된 경우 PyPi 를 통해 설치할 수 있다.
...
모델을 처음 로딩하면 내부적으로 모델을 컴파일하고 최적화하는 과정을 수반한다.
이 과정을 모델에 따라 수 초에서 수 십초 가량 걸리는데 모델을 최초 로딩할 때만 수행된다.
...
모델을 로딩하고 난 뒤에는 로딩된 모델에 정보를 출력해볼 수 있다. 
``Inputs``, ``Outputs`` 에서는 입력/출력 텐서 리스트와 각 텐서의 스키마 정보를 출력한다.
...
``session.run(input)`` 호출은 NPU를 이용하여 추론 실행하고 텐서 리스트를 반환한다.
반환된 결과가 리스트 이므로 첫번째 tensor의 numpy.ndarray을 얻으려면 아래와 같이 실행한다.

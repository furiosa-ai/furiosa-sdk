...
.. note::
  ...
  파이썬 라이브러리를 포함한다. FuriosaAI NPU의 커널 드라이버, 펌웨어 및 런타임은
...
onnxruntime은 `Onnx <https://onnx.ai/>`_ 모델 포맷 지원 및 모델 양자화를 위해 사용된다.
...
위 커맨드로 현재 시스템에 준비되어 있는 Python 버전을 확인할 수 있다. 
만약 Python 커맨드가 존재하지 않거나 하위 버전의 Python을 사용하고 있다면
아래 방법 중에 선택하여 Python 환경을 구성할 수 있다.

* :ref:`CondaInstall`: 우선 권장하는 방법이다.
  `Conda <https://docs.conda.io/projects/conda/en/latest/index.html>`_ 는 
  특정 Python 어플리케이션만을 위한 전용 Python 환경을 구성할 수 있게 해준다.
  따라서 Python 어플리케이션을 설치할 때 종종 발생하는 패키지 의존성 문제나 Python 버전 문제를 
  사용자가 겪지 않도록 한다.
* :ref:`SetupPythonOnUbuntu`: Linux 시스템에서 Python 실행환경을 잘 이해하고 있고
  시스템에 직접 Python 환경을 구성하고 싶다면 선택해도 좋다.
...
Anaconda 설치 후에는 독립된 Python 실행 환경을 셋업하고 필요에 따라 활성화 할 수 있다.
...
생성한 Python 3.8 환경은 ``activate`` 커맨드로 활성화 된다.
...
이미 설치했던 패키지는 활성화 후에 다시 설치 하지 않아도 된다.

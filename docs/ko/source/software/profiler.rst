...
이러한 문제를 해결하기 위해 컨텍스트 내에서 일시적으로 프로파일러를 비활성화할 수 있는 기능을 제공한다.
사용자는 프로파일링하기 원치 않는 작업들을 제외함으로써 불필요한 오버헤드를 최소할 수 있다.

아래는 일시적 프로파일러 비활성화 기능을 사용한 예제로 ``profile.pause`` 와 ``profile.resume`` 사이의 warmup 추론들에 대한 트레이스는 생성되지 않는다.
